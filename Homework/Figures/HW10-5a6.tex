\tikzset{every picture/.style={line width=0.75pt}} %set default line width to 0.75pt        

\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
%uncomment if require: \path (0,393); %set diagram left start at 0, and has height of 393

%Shape: Axis 2D [id:dp1655481678492966] 
\draw  (281,257.1) -- (615,257.1)(314.4,33) -- (314.4,282) (608,252.1) -- (615,257.1) -- (608,262.1) (309.4,40) -- (314.4,33) -- (319.4,40)  ;
%Shape: Axis 2D [id:dp38625059588045074] 
\draw  (347.8,257.1) -- (13.8,257.1)(314.4,33) -- (314.4,282) (20.8,252.1) -- (13.8,257.1) -- (20.8,262.1) (319.4,40) -- (314.4,33) -- (309.4,40)  ;
%Straight Lines [id:da7336512866059715] 
\draw    (314.8,59) -- (314.4,257.1) ;
%Straight Lines [id:da18274923539856058] 
\draw    (406.37,256.73) -- (406.37,156.31) ;
%Straight Lines [id:da08038296278804791] 
\draw    (222.43,157.05) -- (406.37,156.31) ;
%Straight Lines [id:da850803385131168] 
\draw [color={rgb, 255:red, 155; green, 155; blue, 155 }  ,draw opacity=1 ] [dash pattern={on 4.5pt off 4.5pt}]  (314.4,257.1) -- (406.37,256.73) ;
%Straight Lines [id:da6449244567198642] 
\draw    (222.43,257.47) -- (222.43,157.05) ;

% Text Node
\draw (616,248.4) node [anchor=north west][inner sep=0.75pt]    {$\omega $};
% Text Node
\draw (290,12.4) node [anchor=north west][inner sep=0.75pt]    {$H( j\omega )$};
% Text Node
\draw (316.4,260.5) node [anchor=north west][inner sep=0.75pt]    {$0$};
% Text Node
\draw (406.37,260.13) node [anchor=north] [inner sep=0.75pt]    {$7500\pi $};
% Text Node
\draw (316.6,154.65) node [anchor=south west] [inner sep=0.75pt]    {$1$};
% Text Node
\draw (222.43,260.87) node [anchor=north] [inner sep=0.75pt]    {$-7500\pi $};


\end{tikzpicture}
